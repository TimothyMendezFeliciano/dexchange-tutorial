...
export const addLiquidity = async (
    ...
    try {
        ...
        let tx = await tokenContract.approve(
            EXCHANGE_CONTRACT_ADDRESS,
            addPairAAmountWei.toString()
        );
        await tx.wait();
        // After the contract has the approval, add the ether and cd tokens in the liquidity
        tx = await exchangeContract.addLiquidity(addPairAAmountWei, {
 ...
 */
export const calculatePairA = async (
    _addEther = "0",
    ...
    const pairATokenAmount = _addEtherAmountWei
        .mul(cdTokenReserve)
        .div(etherBalanceContract);
    return pairATokenAmount;
};