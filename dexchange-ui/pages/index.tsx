...
import { addLiquidity, calculatePairA } from "../utils/addLiquidity";
import {
  getPairATokensBalance,
  getEtherBalance,
  getLPTokensBalance,
...
export default function Home() {
  ...
  const [reservedPairA, setReservedPairA] = useState(zero);
  // Keeps track of the ether balance in the contract
  const [etherBalanceContract, setEtherBalanceContract] = useState(zero);
  // cdBalance is the amount of `CD` tokens help by the users account
  const [pairABalance, setPairABalance] = useState(zero);
  ...
  const [addPairATokens, setAddPairATokens] = useState(zero);
  ...
  const [removePairA, setRemovePairA] = useState(zero);
  ...
  const getAmounts = async () => {
    try {
      ...
      const _cdBalance = await getPairATokensBalance(provider, address);
      ...
      setPairABalance(_cdBalance);
      setLPBalance(_lpBalance);
      setReservedPairA(_reservedCD);
      setReservedPairA(_reservedCD);
  ...
  const _addLiquidity = async () => {
    try {
      ...
      if (!addPairATokens.eq(zero) && !addEtherWei.eq(zero)) {
        const signer = await getProviderOrSigner(true);
        setLoading(true);
        // call the addLiquidity function from the utils folder
        await addLiquidity(signer, addPairATokens, addEtherWei);
        setLoading(false);
        // Reinitialize the CD tokens
        setAddPairATokens(zero);
  ...
  const _getTokensAfterRemove = async (_removeLPTokens: any) => {
    try {
      ...
      const pairATokenReserve = await getReserveOfCDTokens(provider);
      // call the getTokensAfterRemove from the utils folder
      // @ts-ignore
      const { _removeEther, _removePairA } = await getTokensAfterRemove(
          provider,
          removeLPTokenWei,
          _ethBalance,
          pairATokenReserve
      ...
      return (
          <div>
            <div className={styles.description}>
              ...
              {utils.formatEther(pairABalance)} Pair A Tokens
              <br />
              {utils.formatEther(ethBalance)} Ether
              <br />
              {utils.formatEther(lpBalance)} Pair A LP tokens
            </div>
            <div>
            ...
            we calculate based on the `Eth` amount specified by the user how much `CD` tokens can be added */}
              {utils.parseEther(reservedPairA.toString()).eq(zero) ? (
                  <div>
                    <input
                        type="number"
                    ...
                    <input
                        type="number"
                        placeholder="Amount of ChainWave tokens"
                        onChange={(e) =>
                            setAddPairATokens(
                  ...
                  <div>
                    <input
                        ...
                        onChange={async (e) => {
                          ...
                          const _addCDTokens = await calculatePairA(
                              e.target.value || "0",
                              etherBalanceContract,
                              reservedPairA
                          );
                          setAddPairATokens(_addCDTokens);
                        }}
                        className={styles.input}
                    />
                    <div className={styles.inputDiv}>
                      {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
                      {`You will need ${utils.formatEther(addPairATokens)} Chainwave
                  Tokens`}
                    </div>
                    <button className={styles.button1} onClick={_addLiquidity}>
                      Add
              ...
              <div>
                ...
                <div className={styles.inputDiv}>
                  {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
                  {`You will get ${utils.formatEther(removePairA)} Pair A
      ...
      return (
          <div>
            ...
            >
              ...
              <option value="cryptoDevToken">Pair A Token</option>
            </select>
            <br />
            <div className={styles.inputDiv}>
              {/* Convert the BigNumber to string using the formatEther function from ethers.js */}
  ...
  return (
      <div>
        ...
        <div className={styles.main}>
          <div>
            <h1 className={styles.title}>Welcome to TimoSwap!</h1>
            <div className={styles.description}>
              Exchange Ethereum &#60;&#62; ChainWave Tokens. Known as Pair A.
            </div>
            <div>
              <button
        ...
        <footer className={styles.footer}>
          Made with &#10084; by Timothy Mendez Feliciano as Practice
        </footer>
      </div>
  );
}